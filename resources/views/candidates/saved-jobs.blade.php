<x-app-layout>
    ...
    <div class="grid grid-cols-12 gap-x-5 gap-y-4">
        ...
        @if (session('status'))
            <script>document.addEventListener('DOMContentLoaded',function(){if(window.Swal){Swal.fire({icon:'success',title:'Saved Jobs',text:@json(session('status')),timer:2200,showConfirmButton:false});}});</script>
        @endif
                        ...
                        @foreach($savedJobs as $saved)
                            ...
                            <div class="saved-job-item flex items-center gap-3 p-3 rounded-lg border border-gray-100 hover:border-indigo-100 transition-all"
                                ...
                                <div style="display:flex;gap:4px;flex-shrink:0">
                                    @if($status)
                                        <a href="{{ route('candidate.applications.index') }}" class="cd-btn cd-btn-outline cd-btn-sm" title="View application"><i class="ri-eye-line"></i></a>
                                    @else
                    ...
                    if (action === 'remove') {
                        Swal.fire({icon:'warning',title:'Remove '+ids.length+' saved job(s)?',showCancelButton:true,confirmButtonText:'Yes, remove',confirmButtonColor:'#dc2626'}).then(function(result){
                            if(result.isConfirmed){
                                fetch('{{ route("candidate.saved-jobs.bulk-remove") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'},body:JSON.stringify({ids:ids})})
                        ...
                        Swal.fire({icon:'question',title:'Apply to '+applyableIds.length+' job(s)?',showCancelButton:true,confirmButtonText:'Yes, apply',confirmButtonColor:'#3b82f6'}).then(function(result){
                            if(result.isConfirmed){
                                fetch('{{ route("candidate.saved-jobs.bulk-apply") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'},body:JSON.stringify({ids:applyableIds})})
